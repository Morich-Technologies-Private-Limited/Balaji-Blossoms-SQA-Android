 ...
 *
 * Data: there is only one list endpoint on the server — GET /findByUnitId.
 * Both modes fetch that list. In "user" mode the rows are then filtered
 * client-side to the signed-in user's assignedUserName, since there is no
 * fetch-by-assigned-user endpoint.
 *
...
export default function InvoiceList({
  ...
  const load = useCallback(async () => {
    ...
    /* Both modes read from the unit list; the unit id is required either way. */
    if (!resolvedUnitId) {
    ...
    if (response?.status === "SUCCESS") {
      ...
      /* No per-user endpoint exists — filter the unit list down to the
         signed-in user's own invoices in "user" mode. */
      const scoped =
        mode === "user"
          ? payload.filter((inv) => inv.assignedUserName === resolvedUserName)
          : payload;
      setInvoices(scoped);
    ...
    setLoading(false);
    setRefreshing(false);
  }, [mode, userReady, resolvedUserName, resolvedUnitId]);
